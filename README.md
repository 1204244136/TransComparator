...
预筛选模块的“相似度预筛”阈值控制相似度预筛的通过线（默认 92%）。点击“开始预筛”后，工作台会自动启用“预筛结果”筛选，仅显示本次规则通过或相似度通过的行；结构化冲突不会混入待确认结果。点击“确认预筛”会先统计当前筛选结果中的规则通过数和相似度通过数，经确认后再为这些行勾选“人工确认”。预筛结果会实时写入当前项目快照的备注空间，保存在浏览器 `localStorage`；已人工确认的行会被跳过。运行 AI 校对时，AI 会跳过已通过预筛的行，只对剩余行调用模型。
...
带有 `revisedText` 的 AI 结果会在“校对备注”中显示为可编辑文本框。修改会实时保存到当前项目的浏览器备注空间，差异预览和“导出修改结果”都会使用人工编辑后的文本；三语模式还会保留 AI 实际选择的 B/C 修改列，导出时只替换对应列。
