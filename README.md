...
选择 `gpt-*` 系列模型时，工作台会显示“推理强度”，可使用模型默认值，或选择 `none`、`minimal`、`low`、`medium`、`high`、`xhigh`、`max`。所选值通过 Chat Completions 的 `reasoning_effort` 参数发送；各 GPT 模型支持的档位不同，不支持的值会由接口返回错误。
