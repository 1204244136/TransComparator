...
每次生成会得到一个“项目快照”。快照包含当前对齐后的行表签名、生成时间和项目身份，并写入 `out/translation-compare.json` 与 `out/translation-compare.html`。工作台只展示一个项目快照，不承担选择文件或切换项目的职责。

控制台由 `npm run setup` 启动，负责选择输入、保存设置、运行生成流程和承载 AI 校对 API。开始生成新的工作台时，控制台会清空上一快照的 AI 运行状态；如果 AI 校对仍在运行，控制台会拒绝生成，避免运行态跨项目。
...
- `原文`：源语言文本。
- `非原文 A`：第一份译文、校订本或地区版本。
- `非原文 B`：第二份译文、校订本或地区版本。

保存选择后，点击页面里的生成按钮。本地服务会依次运行段落导出、跨语言对齐和工作台构建，完成后提供 `translation-compare.html` 入口。
...
`translation-compare.html` 可以直接用浏览器打开。备注保存在浏览器本地，不会写回源文本。
...
启动前选择校对目标列、并发数和预筛选阈值。程序会先把两份非原文极度相似的行标记为已处理，再对剩余行按多并发、短对话逐行调用模型。仅凭当前行不足以判断时，模型只需返回 `needsContext=true`；工作台后端会按当前项目快照每轮固定补充当前行前一行和后一行，最多 6 轮、12 行。AI 结果会实时写入当前项目快照的备注空间，仍保存在当前浏览器 `localStorage`。
